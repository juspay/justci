...
module JustCI.Verdict
  ( -- * Outcome values
    ...
    -- agreement check against 'JustCI.CommitStatus.terminalToCommitStatus'
...
-- 'JustCI.Pipeline'; this module no longer does its own 'parseNodeId'
-- call. That keeps the two consumers of the state stream ('JustCI.CommitStatus'
...
-- threads this in from the loaded 'JustCI.Hosts.Hosts' so the verdict
...
-- "JustCI.Hosts" vocabulary directly.
--
