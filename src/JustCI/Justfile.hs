...
-- recipe is the pipeline root) lives in @JustCI.Root@ — that axis of change is
-- independent of the wire format and shouldn't ride along here.
module JustCI.Justfile
  ( -- * Schema
    RecipeName,
    recipeNameFromText,
    Recipe (..),
...
import JustCI.Subprocess (SubprocessError, runSubprocess)
...
-- the typed operations below ('recipeCommand', or 'JustCI.Nix.realisedJust'
-- for remote lanes).
justBin :: FilePath
...
-- identifier (e.g. the @name@ field of a 'JustCI.ProcessCompose.Events.ProcessState')
