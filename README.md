...
**Per-repo overrides.** An optional `.justci/hosts.json` at the repo root, identical schema to the global file, overrides the global file *for that checkout*. Useful when a project has its own builders distinct from a contributor's defaults, or when the routing should travel with the code (every contributor cloning the repo gets the same fanout). Resolution order is **global ◁ repo ◁ CLI** — rightmost wins. Missing entries cascade: a platform configured only globally still routes via the global host. The repo file is optional; absence is identical to today's behaviour.

**One-shot CLI overrides.** A repeatable `--host PLATFORM=ADDR` option on `justci run` overlays onto whatever the file layers resolve to, with CLI entries winning on collision: `justci run --host x86_64-linux=root@lxc-foo` redirects the linux lane to a throwaway LXC container for that invocation without touching either JSON file. Platforms not named on the CLI still consult the file layers as usual.
